<template>
  <!-- Inline SVG (vs <img src="logo.svg">) so we can repaint the dark
       text to white in dark mode without touching the orange mountain.
       The two paths are split by their original fills: #262323 → text,
       #f93 → mountain. CSS classes drive theme-aware colors. -->
  <svg
...
<style lang="scss">
// Out-specifies the scoped rule above via the html[data-theme] selector.
// In dark mode the textual ".org" + the camp-to-camp logotype turn white
// so they read on the dark top bar. The orange mountain glyph keeps its
// brand color in both themes.
html[data-theme='dark'] .logo-ctc .logo-text {
  fill: #f5f5f5;
}
</style>
