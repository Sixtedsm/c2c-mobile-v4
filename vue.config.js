...
const result = {
  ...
  pwa: {
    ...
    workboxOptions: {
      ...
      exclude: [/\.map$/, /^manifest.*\.js$/, /\.pdf$/, /robots\.txt$/, /revive-adserver\.html$/],
    },
    iconPaths: {
...
} else if (process.env.BUILD_ENV === 'github-v3') {
  // V3 deploys to https://sixtedsm.github.io/c2c-mobile-v3/ — a single
  // sub-path, no per-branch fan-out. Hash routing so deep links resolve
  // without server-side rewrites on GitHub Pages.
  config.routerMode = undefined;
  result.publicPath = '/c2c-mobile-v3/';
  result.configureWebpack.performance.hints = 'warning';
} else if (process.env.BUILD_ENV === 'github-v4') {
  // V4: refonte terrain. Mini-app dédiée par topo offline (5 vues
  // swipables) + planificateur de treks. Deploys to
  // https://sixtedsm.github.io/c2c-mobile-v4/. Hash routing, single
  // sub-path. V3 reste en parallèle pour continuité.
  config.routerMode = undefined;
  result.publicPath = '/c2c-mobile-v4/';
  result.configureWebpack.performance.hints = 'warning';
